...
@section('content')
    ...
    <div class="content-wrapper">
        ...
        <div class="container-xxl flex-grow-1 container-p-y">
            <h4 class="fw-bold py-3 mb-4" style="color: #124E66;"><span class="text-muted fw-light"></span>Edit Candidate</h4>
                    ...
                    <div class="col-xxl">
                        <div class="card mb-4">
                            <div class="card-header d-flex align-items-center justify-content-between">
                                <h5 class="mb-0">Edit {{$candidate->first_name}}'s information</h5>
                                {{--                                <small class="text-muted float-end">Merged input group</small>--}}
                            </div>
                            <div class="card-body">
                                <form action="{{url('/update_candidate')}}" method="post">
                                    @csrf
                                    <div class="row mb-3">
                              ...
                              ><i class="bx bx-id-card"></i
                                  ></span>
                                                <input
                                                    ...
                                                    aria-describedby="basic-icon-default-fullname2"
                                                    name="date_of_birth"
                                                    value="{{\Carbon\Carbon::parse($candidate->date_of_birth)->format('Y-m-d')}}"
                                                    required
                                                />
                                            </div>
                                        </div>
                                    ...
                                    <input type="text" value="{{$candidate->id}}" name="id" hidden>
                                  ...
                                  ></span>
                                                <select class="dropdown form-control" name="party_id" required>
                                                    ...
                                                    @foreach($parties as $party)
                                                        <option value="{{$party->id}}" {{$candidate['party'] ? $candidate['party']['id'] == $party->id ? 'selected' : "" : "" }} class="dropdown-item">{{$party->name}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                  ...
                                  ></span>
                                                <select class="dropdown form-control" name="area_of_election_id" required>
                                                    ...
                                                    @foreach($area_of_elections as $area_of_election)
                                                        <option value="{{$area_of_election->id}}" {{$candidate['area'] ? $candidate['area']['id'] == $area_of_election->id ? 'selected' : "" : "" }} class="dropdown-item">{{$area_of_election->areatype . " " . $area_of_election->area_no}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
